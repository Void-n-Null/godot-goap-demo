...
public sealed class MateAction(float interactionTime = 2.5f, float requiredDistance = 96f) : PeriodicGuardAction(0.25f)
{
    ...
    public override ActionStatus Update(Entity agent, float dt)
    {
        ...
        // Verify consent
        if (_partner.TryGetComponent<NPCData>(out var partnerData))
        {
            if (partnerData.IncomingMateRequestStatus == NPCData.MateRequestStatus.Rejected)
            {
                Fail("Partner rejected request");
                return ActionStatus.Failed;
            }

            if (partnerData.IncomingMateRequestStatus != NPCData.MateRequestStatus.Accepted)
            {
                // Wait for acceptance... but not forever
                if (_timer > 5.0f) // Reuse timer for timeout check before interaction starts
                {
                    Fail("Timed out waiting for partner acceptance");
                    return ActionStatus.Failed;
                }
                _timer += dt;
    ...
    {
        LM.Error($"MateAction fail: {reason}");
        _failed = true;
    }
}
