...
public partial class GameManager : SingletonNode<GameManager>
{
	...
	/// </summary>
	private static Dictionary<string, Type> DiscoverScenarios()
	{
	...
	private void LoadScenario(string scenarioName)
	{
		...
		GD.Print($"Available scenarios: {string.Join(", ", availableScenarios.Keys)}");

		// Try to find and instantiate the requested scenario
		if (!availableScenarios.TryGetValue(scenarioName, out var scenarioType))
		{
			GD.PrintErr($"Scenario '{scenarioName}' not found. Falling back to 'Default'.");
			scenarioType = availableScenarios.GetValueOrDefault("Default");
		}

		if (scenarioType == null)
		{
			GD.PrintErr("No scenarios found! Please create at least one scenario class.");
			return;
		}

		var scenario = (Scenario)Activator.CreateInstance(scenarioType);
		GD.Print($"Loading scenario: {scenario.Name}");
		GD.Print($"Description: {scenario.Description}");
		scenario.Setup();
	}
